...
static void makeKeywords(){
	...
	alphaTokes["Asm"] = ASM;                    //Emil  inlin asm 8/10/2018
	alphaTokes["AsmInc"] = ASMINC;                    //Emil  inlin asm 9/10/2018
	alphaTokes["AsmData"] = ASMDATA;                    //Emil  inlin asm 9/10/2018
...
void Toker::nextline(){
	...
	for( int k=0;k<line.size(); ){
		...
		if (c == '/' &&  line[k+1]=='/') {                           //Emil 5/10/2018  ' c/c++ comment
			for (++k; line[k] != '\n'; ++k) {}
			continue;
		}
		...
		if (c == '!'&&n == '=' ) {                     //Emil 5/10/2018  ' c/c++ not equ
			tokes.push_back(Toke(NE, from, k += 2));
			continue;
		}
		if( (c=='<'&&n=='=')||(c=='='&&n=='<') ){
